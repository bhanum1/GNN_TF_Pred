...
def inner_loop(model, inner_lr, loader, grad_steps, eval= False):
    ...
    #support = random.sample(range(50), 10)
    #query = random.sample(range(50), 10)
...
def outer_loop(model, num_epochs, optimizer, train_loaders, inner_lr):
    for epoch in range(num_epochs):
        ...
        if epoch == 0 or (epoch+1) % 100 == 0:
            print("{0} Train Loss: {1:.3f}".format(epoch, total_loss.cpu().detach().numpy() / num_tasks))
...
def eval(model, fine_lr, test_loaders, test_tasks):
    ...
    for task in range(len(test_loaders)):
        ...
        for i in range(len(pred)):
            pred[i] = pred[i][0].cpu().detach().numpy()[0]
        for i in range(len(target)):
            target[i] = target[i][0].cpu().detach().numpy()[0]

        pred_out.extend(pred)
        target_out.extend(target)

        MAE = np.average(abs(np.array(pred_out) - np.array(target_out)))


        print(task, MAE, round(scipy.stats.spearmanr(np.array(pred_out), np.array(target_out))[0],3))
